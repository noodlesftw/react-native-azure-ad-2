...
export default class AzureLoginView extends React.Component {
	...
	_handleNavigationStateChange(e: any) {
		...
		if (handleNavigationStateChange) {
			onNavigationStateChange(e)
		}

		this._handleTokenRequest(url)
	}

	_handleTokenRequest(url: string): any{
		let code = /((\?|\&)code\=)[^\&]+/.exec(url)

		// get code when url chage
		if (code) {
			const code = String(code[0]).replace(/(\?|\&)?code\=/, '')
		...
		if (!this.state.cancelled && this.props.onCancel && url.indexOf('error=access_denied') > -1) {
